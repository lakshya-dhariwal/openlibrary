...
const RENDER_AUTOCOMPLETE_RESULT = {
  ["/search"](work) {
    ...
    return `
            <li>
                <a href="${work.key}">
                    <img src="//covers.openlibrary.org/b/id/${
                      work.cover_i
                    }-S.jpg?default=https://openlibrary.org/static/images/icons/avatar_book-sm.png" alt=""/>
                    <span class="book-desc">
                        <div class="book-title">${websafe(
                          work.title
                        )}</div><div class="book-subtitle">${websafe(
      work.subtitle
    )}</div> by <span class="book-author">${websafe(author_name)}</span>
                    </span>
                </a>
            </li>`;
  },
    ...
    return `
            <li>
                <a href="/authors/${author.key}">
                    <img src="//covers.openlibrary.org/a/olid/${
                      author.key
                    }-S.jpg?default=https://openlibrary.org/static/images/icons/avatar_author-lg.png" alt=""/>
                    <span class="author-desc"><div class="author-name">${websafe(
                      author.name
                    )}</div></span>
                </a>
            </li>`;
...
export class SearchBar {
  ...
  submitForm() {
    ...
    console.log({ end: this.facetEndpoint, val: this.$input.val() });
    this.$form.attr(
      "action",
  ...
  static composeSearchUrl(
    ...
    logger.debug(
      "a *******_________++++++++++___________ _________++++++++++____________________++++++++++___________ _________++++++++++___________ \n",
      facetEndpoint,
      1
    );

    console.log({ facetEndpoint, q });
